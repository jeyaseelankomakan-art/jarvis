...
<body>
    ...
    <div class="container">
        ...
        <div class="controls">
            <button class="mode-button" id="continuousBtn">Continuous: Off</button>
    ...
    <script>
        ...
        if (SpeechRecognition) {
            ...
            recognition.onresult = async (event) => {
                ...
                if (continuousMode && !isSpeaking && !isListening) {
                    startListening();
                }
            };
            ...
            recognition.onerror = (event) => {
                ...
                if (continuousMode && event.error !== 'not-allowed' && event.error !== 'service-not-allowed') {
                    setTimeout(() => {
                        if (continuousMode && !isThinking && !isSpeaking && !isListening) {
                            startListening();
                        }
                    }, 800);
                }
            ...
            recognition.onend = () => {
                ...
                if (continuousMode && !isThinking && !isSpeaking) {
                    startListening();
                } else if (!continuousMode && !isThinking && !isSpeaking) {
        ...
        continuousBtn.addEventListener('click', () => {
            ...
            continuousBtn.innerText = continuousMode ? 'Continuous: On' : 'Continuous: Off';

            if (continuousMode) {
                statusText.innerText = 'Continuous mode enabled';
                startListening();
            } else {
        ...
        async function sendToJarvis(prompt) {
            ...
            shouldResumeAfterSpeak = continuousMode;
        ...
        function speak(text) {
            ...
            utterance.onend = () => {
                isSpeaking = false;
                if (shouldResumeAfterSpeak && continuousMode && !isThinking && !isListening) {
                    startListening();
                    shouldResumeAfterSpeak = false;
                    return;
                }
            ...
            utterance.onerror = () => {
                isSpeaking = false;
                if (continuousMode && !isThinking && !isListening) {
                    startListening();
                    return;
                }
                statusText.innerText = "Ready";
